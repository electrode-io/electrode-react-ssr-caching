...
const updateReactId = (r, staticMarkup, hostContainerInfo) => {
  let id = hostContainerInfo._idCounter;
  if (id > 1) {
    r = r.replace(/ data-reactroot=\"[^\"]*\"/, "");
  } else {
    return r;
  }
  if (staticMarkup) {
  ...
  r = r.replace(/data-reactid="([0-9]*)"|react-text: ([0-9]*) /g, (m, a) => {
    return a ? `data-reactid="${id++}"` : `react-text: ${id++} `;
  });
...
const templatePostProcess = (r, lookup, realProps, transaction, hostContainerInfo) => { // eslint-disable-line
  ...
  return updateReactId(r, transaction.renderToStaticMarkup, hostContainerInfo);
};
...
ReactCompositeComponent.Mixin.mountComponentCache = function mountComponentCache(transaction, hostParent, hostContainerInfo, context, parentDebugID) { // eslint-disable-line
  ...
  if (doCache) {
    ...
    if (strategy === "simple") {
      ...
      if (cached) {
        ...
        const r = updateReactId(cached.html, transaction.renderToStaticMarkup, hostContainerInfo);
        return config.debug ?
      ...
      cached = cacheStore.getEntry(name, key);
      if (cached) {
        const r = templatePostProcess(cached.html, template.lookup, saveProps, transaction, hostContainerInfo); // eslint-disable-line
        this.__profileTime(startTime);
  ...
  if (template) {
    ...
    if (template.lookup) {
      r = templatePostProcess(r, template.lookup, saveProps, transaction, hostContainerInfo);
    } else {
      r = updateReactId(r, transaction.renderToStaticMarkup, hostContainerInfo);
    }
