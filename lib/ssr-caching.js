...
ReactCompositeComponent.Mixin.mountComponentCache = function mountComponentCache(transaction, hostParent, hostContainerInfo, context, parentDebugID) { // eslint-disable-line
  ...
  if (template) {
    ...
    cacheStore.newEntry(name, key, {
        html: r.replace(` ${MARKUP_FOR_ROOT}`, ""),
        maxAge: maxAge
    });
...
exports.cacheSize = function () {
  return cacheStore.cache.length;
};

exports.cacheEntries = function () {
  return cacheStore.cache.itemCount;
};

exports.cacheHitReport = function () {
  _.forEach(cacheStore.hits, (hits, key) => {
    console.log(`Cache Entry ${key} Hits ${hits}`); // eslint-disable-line
  });
};
